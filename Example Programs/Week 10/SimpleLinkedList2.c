...
void reverse()
{
	...
	if(head == NULL)	// With 0 nodes
	{
		printf("The linked list is empty\n");
		return;
	}
	else if(head->next == NULL) // With 1 node
	{
		printf("The linked list just has one node\n");
		return;
	}
	else if(head->next->next == NULL) // With 2 nodes
	{
		temp = head;
		head = head->next; // head points to second node now
		head->next = temp; // the second node's next, points to the first node now
		temp->next = NULL; // the first node's next points to NULL now
		// The list is now changed from A ---> B to B ---> A
	}
	else // With more than 2 nodes
	{
		temp = head;
		next1 = head->next;
		next2 = next1->next;
		...
		// A ---> B ---> C ---> D ---> E
		// then temp is at A, next1 is at B and next2 is at C
		head->next = NULL; // This will now be the last node in the list
		while(next2 != NULL)
		{
			// flip the pointers between temp and next1
			next1->next = temp;
			// move pointers
			temp = next1;
			next1 = next2;
		...
		next1->next = temp;
		head = next1; // This is the new head now
	}
	printf("List reversal operation successful\n");
}
